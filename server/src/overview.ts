    ...
    await Promise.all([
      db.execute<CountRow>(sql`
        SELECT
          ...
          (SELECT count(*)::int FROM "auth"."session" WHERE "expiresAt" > now()) AS active_sessions,
          (SELECT count(*)::int FROM "auth"."user" WHERE COALESCE("twoFactorEnabled", false)) AS mfa_users,
          (SELECT count(DISTINCT "userId")::int FROM "auth"."passkey") AS passkey_users,
          (SELECT count(*)::int FROM "auth"."apikey" WHERE COALESCE(enabled, true) AND ("expiresAt" IS NULL OR "expiresAt" > now())) AS active_api_keys,
          (SELECT count(*)::int FROM "auth"."apikey" WHERE "expiresAt" BETWEEN now() AND now() + interval '7 days') AS expiring_api_keys
      `),
      ...
      db.execute<TrendRow>(sql`
        ...
        FROM days
        LEFT JOIN "auth"."audit_events" a ON a.created_at >= days.day AND a.created_at < days.day + interval '1 day'
